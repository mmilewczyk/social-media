...
public record ValidatorService(PasswordValidator passwordValidator,
    ...
    public void validatePasswordAndEmail(RegistrationRequest request) {
        ...
        if (!isValidEmail) throw new IllegalStateException(String.format("Email %s is not valid!", request.email()));
    ...
    public void checkIfEmailOrUsernameAreNotTaken(User user) {
        ...
        if (userExists) throw new IllegalStateException(String.format("Username %s already taken", user.getUsername()));
        if (emailExists) throw new IllegalStateException(String.format("Email %s already taken", user.getEmail()));
    }
}
